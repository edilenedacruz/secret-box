...
We’re going to start from the end of the Building and Testing with Express lesson
...
Based on that environment, we’ll fetch the database configuration from knexfile.js for whatever environment we’re in.
process.exit() just tells node that we’re done.
...
Also try throwing in a debugger; and then run node debug database-spike.js. Here’s a quick intro to debugging in node
...
Let’s make the assertions more explicit. We’re building a JSON API after all.
We’re using integer ids instead of string ids.
...
// In the Procfile
web: node server.js
Node applications don’t come with databases by default. We’ll have to manually add it to our application:
...
Check this stack overflow question for a good way to get the id for a newly created record
